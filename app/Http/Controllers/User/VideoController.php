...
class VideoController extends CommonController
{
    public function index(Video $video)
    {
        ...
        $data = $video->limit($offset, $pageSize)->orderBy('created_at', 'DESC')->get(['id', 'title', 'author', 'show', 'views', 'created_at']);

        $total = $video->count();
        $pageHtml = Page::html('?page=', $total, $page, $pageSize);
        $this->assign('video', $data);
        $this->assign('page_html', $pageHtml);
        return $this->fetch('user/video_list');
    }
    ...
    public function edit(Video $video, Category $category)
    {
        ...
        return $this->fetch('user/video_edit');
    }

    public function save(Video $video, HTMLPurifier $purifier)
    {
